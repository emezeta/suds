...
2.  Using `easy-install`
	...
	\-   Run `easy\_install suds\-community`.
...
You can inspect service object with: `**str()**` as follows to get a
...
\... and invoke our method named addPerson() as follows:
	...
	try:
	   ...
	   person\_added = client.service.addPerson(person)
...
* xstq:: The XML schema type qualified flag indicates that `xsi:type` attribute **values** should be qualified by namespace.
...
Both the `service` **and** `port` may be subscripted
...
<!-- -->
	...
	client.set\_options(service='OtherBLZService', port='soap')
...
`soapheaders` `option` as follows:
	...
	client.set\_options(soapheaders=token)
...
OR
	...
	client.set\_options(soapheaders=(userid,password))
...
pass None place holders. This works much like the method parameters. Eg:
	...
	client.set\_options(soapheaders=myheaders)
...
`Elements` as follows:
	...
	client.set\_options(soapheaders=ssn)
...
`UsernameToken` with `clear-text` password (no digest).
	...
	client.set\_options(wsse=security)
...
could be implemented as follows:
	...
	passman.add\_password(None, baseurl, username, password)

	authhandler = urllib2.HTTPBasicAuthHandler(passman)

	client.options.transport.urlopener  = urllib2.build\_opener(authhandler)
...
`users to install the `python-ntlm <http://code.google.com/p/python-ntlm/>`_. It is **not** packaged with suds.
...
Sending a raw soap message:
	...
	print client.service.test(\_\_inject={'msg':message})
...
Injecting a response for testing:
	...
	print client.service.test(\_\_inject={'reply':reply})
...
class UnverifiedHttpsTransport(suds.transport.http.HttpTransport):
	...
	def \_\_init\_\_(self, \*args, \*\*kwargs):
		...
		super(UnverifiedHttpsTransport, self).\_\_init\_\_(\*args, \*\*kwargs)
...
class ClientHttpsTransport(HttpTransport):
	...
	def \_\_init\_\_(self, certfile, keyfile, cafile, \*args, \*\*kwargs):

		super(ClientHttpsTransport, self).\_\_init\_\_(\*args, \*\*kwargs)
	...
	def u2handlers(self):
		...
		context = ssl.create\_default\_context(ssl.Purpose.SERVER\_AUTH, cafile=self.cafile)

		context.load\_cert\_chain(self.certfile, self.keyfile)
...
custom*https = ClientHttpsTransport('/path/to/certificate*file', '/path/to/key*file', '/path/to/ca*file')
...
and XSD schemas but does **not** apply to service method invocation
...
So, uses may plug-in any kind of cache they want.
	...
	client.set\_options(cache=MyCache())

To disable caching:

	#!python

	client.set\_options(cache=None)
